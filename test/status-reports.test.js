...
test('status report mensal separa formulario, parametros e gestao', () => {
  ...
  assert.doesNotMatch(indexSource, /data-status-report-panel=/);
  ...
  assert.match(indexSource, /<section class="view" id="statusReports">/);
  assert.match(indexSource, /<section class="view" id="statusReportParameters">/);
  ...
  assert.match(indexSource, /app\.js\?v=20260813-faturamento-result-gross-margin/);
  assert.match(indexSource, /styles\.css\?v=20260813-faturamento-result-gross-margin/);
  ...
  assert.match(appSource, /statusReportParameters: 'Contratos\/Par.metros Status'/);
  assert.match(appSource, /statusReportManagement: 'Contratos\/Gest.o dos Status'/);
  assert.match(appSource, /view: 'statusReportParameters'/);
  assert.match(appSource, /view: 'statusReportManagement'/);
  assert.match(appSource, /if \(viewId === 'statusReportParameters'\) return isCurrentUserAdmin\(\)/);
  assert.match(appSource, /if \(viewId === 'statusReportManagement'\) return isCurrentUserAdmin\(\)/);
