...
export function normalizeCandidate(candidate) {
  ...
  return {
    ...
    rejectedAt: String(candidate.rejectedAt ?? candidate.reprovado_em ?? '').trim(),
    rejectedBy: String(candidate.rejectedBy ?? candidate.reprovado_por ?? '').trim(),
    rejectionReason: String(candidate.rejectionReason ?? candidate.motivo_reprovacao ?? '').trim(),
...
export function normalizeCvFilter(filter) {
  ...
  return {
    ...
    mandatorySkills,
    coreSkill: String(filter.coreSkill || '').trim(),
    technicalSkills: String(filter.technicalSkills || '').trim(),
    desirableSkills: String(filter.desirableSkills || '').trim(),
    locations: String(filter.locations || '').trim(),
    ...
    matchPercent,
    resultLimit,
    searchStatus: String(filter.searchStatus || '').trim(),
    searchMessage: String(filter.searchMessage || '').trim(),
    searchSource: String(filter.searchSource || '').trim(),
    searchExecutedAt: String(filter.searchExecutedAt || '').trim(),
    searchJobId: String(filter.searchJobId || '').trim(),
    searchOwnerId: String(filter.searchOwnerId || '').trim(),
    searchResults: Array.isArray(filter.searchResults) ? filter.searchResults.map(normalizeCvSearchResult) : [],
    searchReviewResults: Array.isArray(filter.searchReviewResults) ? filter.searchReviewResults.map(normalizeCvSearchResult) : [],
    searchRejectedResults: Array.isArray(filter.searchRejectedResults) ? filter.searchRejectedResults.map(normalizeCvSearchResult) : [],
    searchStats: filter.searchStats && typeof filter.searchStats === 'object' ? filter.searchStats : undefined,
    searchSourceStats: filter.searchSourceStats && typeof filter.searchSourceStats === 'object' ? filter.searchSourceStats : undefined,
    searchApprovedCount: Number(filter.searchApprovedCount || 0),
    searchRecommendedCount: Number(filter.searchRecommendedCount || 0),
    searchStopReason: String(filter.searchStopReason || '').trim(),
...
export function normalizeCvSearchResult(result) {
  ...
  return {
    ...
    classification: ['approved', 'review', 'rejected'].includes(result.classification) ? result.classification : (/revisar/i.test(result.source || '') ? 'review' : 'approved'),
    scoreType: String(result.scoreType || 'evidência do currículo'),
    pendingChecks: Array.isArray(result.pendingChecks) ? result.pendingChecks : [],
    operationalChecks: Array.isArray(result.operationalChecks) ? result.operationalChecks : [],
    approvalScope: String(result.approvalScope || ''),
    presentationStatus: String(result.presentationStatus || 'requires_validation'),
    technicalScore: Math.max(0, Math.min(100, Number(result.technicalScore) || 0)),
    triageGroup: String(result.triageGroup || 'validação documental pendente'),
